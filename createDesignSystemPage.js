...
fs.writeFileSync(path.join(__dirname, 'frontend/src/pages/DesignSystem.jsx'), pageContent.trim());

// Update router
const routerPath = path.join(__dirname, 'frontend/src/routes/index.jsx');
let routerCode = fs.readFileSync(routerPath, 'utf8');

// If not already imported, import it
if (!routerCode.includes('DesignSystem')) {
  // Add it to lazy imports
  routerCode = routerCode.replace(
    'const NotFound = lazy(() => import(\'../pages/NotFound\'));',
    'const NotFound = lazy(() => import(\'../pages/NotFound\'));\nconst DesignSystem = lazy(() => import(\'../pages/DesignSystem\'));'
  );

  // Add route inside public routes or at root level. We will add it at the top level outside Layouts just for demo.
  const routeString = `{ path: '/design-system', element: <Suspense fallback={<div>Loading...</div>}><DesignSystem /></Suspense> },\n      { path: '*', element: <NotFound /> }`;
  routerCode = routerCode.replace('{ path: \'*\', element: <NotFound /> }', routeString);
  fs.writeFileSync(routerPath, routerCode);
}

console.log('Successfully created DesignSystem page and added to routes');
